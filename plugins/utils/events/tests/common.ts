...
function rimrafSync(p) {
	...
	try {
		...
		if (e.code === 'ENOENT') return;
		if (e.code === 'EPERM') return rmdirSync(p, er);
...
if (process.env.TEST_THREAD_ID) {
	...
	if (!process.env.NODE_COMMON_PORT) exports.PORT += +process.env.TEST_THREAD_ID * 100;
...
Object.defineProperty(exports, 'inFreeBSDJail', {
	get: function () {
		...
		if (
			...
			child_process.execSync('sysctl -n security.jail.jailed').toString() === '1\n'
		) {
			inFreeBSDJail = true;
		} else {
			inFreeBSDJail = false;
...
Object.defineProperty(exports, 'opensslCli', {
	get: function () {
...
exports.hasIPv6 = Object.keys(ifaces).some(function (name) {
	return (
		/lo/.test(name) &&
		ifaces[name].some(function (info) {
...
exports.platformTimeout = function (ms) {
	...
	if (process.config.variables.arm_version === '6') return 7 * ms; // ARMv6
...
function leakedGlobals() {
	...
	for (var val in global) if (-1 === knownGlobals.indexOf(global[val])) leaked.push(val);
