...
export const UtilDex = new (class {
	teamToJSON(text: string | any) {
		text = text.split('\n');
		let team = [];
	...
	packTeam(team: AnyObject[]) {
		...
		for (let i = 0; i < team.length; i++) {
			...
			if (set.ivs) {
				ivs =
					'|' +
					(set.ivs['hp'] === 31 || set.ivs['hp'] === undefined ? '' : set.ivs['hp']) +
					',' +
	...
	exportTeam(team: AnyObject[] | string) {
		if (!team) return '';
		...
		for (let i = 0; i < team.length; i++) {
			...
			if (curSet.name !== curSet.species) {
				text += '' + curSet.name + ' (' + (this.getTemplate(curSet.species).name || curSet.species) + ')';
			} else {
				text += '' + (this.getTemplate(curSet.species).name || curSet.species);
			}
			if (curSet.gender === 'M') text += ' (M)';
			if (curSet.gender === 'F') text += ' (F)';
			if (curSet.item) {
				curSet.item = this.getItem(curSet.item).name || curSet.item;
				text += ' @ ' + curSet.item;
			}
			text += '\n';
			if (curSet.ability) {
			...
			if (curSet.ivs) {
					...
					if (move.substr(0, 13) === 'Hidden Power ' && move.substr(0, 14) !== 'Hidden Power [') {
						...
						for (let stat in Chart.StatsName) {
							if (
								...
								(Chart.Type[hpType].HPivs[stat] || 31)
							) {
								defaultIvs = false;
								break;
							}
				...
				if (!defaultIvs) {
					for (let stat in Chart.StatsName) {
						let curStat = curSet.ivs[stat];
						if (typeof curStat === 'undefined' || isNaN(curStat) || curStat === 31) continue;
						if (first) {
							text += 'IVs: ';
							first = false;
						} else {
			...
			if (curSet.moves) {
				for (let j = 0; j < curSet.moves.length; j++) {
					...
					text += '- ' + (this.getMove(move).name || move) + '\n';
				}
			}
			text += '\n';
		}
	...
	fastUnpackTeam(buf: string) {
		...
		while (true) {
			...
			set.ability =
				template.abilities && ability in {'': 1, 0: 1, 1: 1, H: 1}
					? template.abilities[ability || '0']
					: ability;
			i = j + 1;
	...
	teamOverview(buf: string) {
		let team = this.fastUnpackTeam(buf);
