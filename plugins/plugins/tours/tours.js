...
exports.commands = {
	...
	tourend(target, room, user) {
		...
		if (this.cmd === 'tourstart' && !tourData[this.id][room.id].signups) return this.sendReply(Lang.getSub(this.lang, 'tourend', 'err2'));
	...
	tournament (target, room, user) {
		...
		if (target) {
			...
			if (params.format) {
				var format = this.bot.parseAliases(params.format);
	...
	official (target, room, user) {
		...
		if (!Leaderboards.isConfigured(this.id, room.id)) return this.sendReply(Lang.replaceSub(this.lang, 'official', 'not', room.title));
		if (!tourData[this.id][room.id]) return this.sendReply(Lang.getSub(this.lang, 'official',"notour"));
		if (this.cmd === "unofficial") {
			if (!tourData[this.id][room.id].isOfficialTour) return this.sendReply(Lang.getSub(this.lang, 'official',"already-not"));
			tourData[this.id][room.id].isOfficialTour = false;
			this.sendReply(Lang.getSub(this.lang, 'official', "unofficial"));
		} else {
			if (tourData[this.id][room.id].isOfficialTour) return this.sendReply(Lang.getSub(this.lang, 'official',"already"));
